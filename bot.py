# -*- coding: utf-8 -*-
# Secure template bot.py — placeholders used for sensitive values
# Features:
# - Flask keep-alive (managed to avoid port conflicts)
# - Malware scan + owner approval (Approve/Quarantine)
# - Owner/Admin-only "All User Files" menu button
# - Robust process starter with logging for user scripts
# - ZIP handling and saving metadata to sqlite
...
# telegram - install pyTelegramBotAPI
import telebot
from telebot import types

# ----------------- Configuration (REPLACE placeholders) -----------------
TOKEN = '8419880200:AAG5OpgB0BG7FOpN-XrUu_7y3hGJKmWimI4'      # <-- replace
OWNER_ID = 7652176329                        # <-- replace (int)
ADMIN_ID = 7652176329                        # <-- replace (int)
YOUR_USERNAME = 'your_username_here'        # <-- replace without @
UPDATE_CHANNEL = 'https://t.me/yourchannel' # <-- replace

# ----------------- Paths & limits -----------------
BASE_DIR = os.path.abspath(os.path.dirname(__file__))
UPLOAD_BOTS_DIR = os.path.join(BASE_DIR, 'upload_bots')
INF_DIR = os.path.join(BASE_DIR, 'inf')
DATABASE_PATH = os.path.join(INF_DIR, 'bot_data.db')
...
bot_scripts = {}         # key -> {process, started_at, logfile}
user_files = {}          # in-memory map user_id -> [(file_name, file_type)]
